...
namespace RTSgame.Levels.PlayableLevels
{
    ...
    {
        public static void LoadLevel(Surroundings surroundings, Player[] players)
        {
            int worldSeed = 0;// EasyRandom.Next(int.MaxValue);
            worldSeed = 0;
            ...
            world.RegionMap.ChangeEveryPoint(delegate(ref Region R)
            {
                ...
                else if (R.GetBlockPoint() == Constants.Player1StartingRegionPoint)
                {
                    BuildingNode mon = new BuildingNode(Calculations.PointToVector2(R.GetCenterOfBlockAsWorldPoint()));
                    gameState.addGameObject(mon);
                    drawManager.addDrawable(mon);
                    MainBase mainBase = new MainBase(mon, true, players[0]);
                    GameState.GetInstance().addGameObject(mainBase);
                    DrawManager.GetInstance().addDrawable(mainBase);
                }
