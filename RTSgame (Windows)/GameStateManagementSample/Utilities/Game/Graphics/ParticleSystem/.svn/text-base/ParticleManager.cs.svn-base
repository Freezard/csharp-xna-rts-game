...
namespace RTSgame.Utilities.Graphics.ParticleSystem
{
    class ParticleManager
    {
        ...
        public void AddExplosion(Vector3 position, Vector3 velocity, float scale)
        {
            ...
            for (int i = 0; i < 30; i++)
                explosion.AddParticle(position, velocity);
            for (int i = 0; i < 50; i++)
                explosionSmoke.AddParticle(position, velocity);
            
            Shaders.GetInstance().AddPointLight(position + new Vector3(0, 0.0f, 0), new Vector4(1, 1, 0, 0), 0.8f, 4, 0.05f);
        }
        ...
        public void AddSpecial(Vector3 position, Vector3 velocity, float scale)
        {
            ...
            Shaders.GetInstance().AddPointLight(position, new Vector4(1, 0, 0, 0), 1, 4, 0.5f);
        }
