...
namespace RTSgame.Utilities
{
    ...
    public class BlockMap<T> : Array2D<T> where T : IBlock , new()
    {
        ...
        public T GetBlockViaWorldPoint(Point p)
        {
            ...
            locationX = Calculations.Clamp(locationX, 0, XDimension - 1, "GetBlock: location is out of map bounds");
            locationY = Calculations.Clamp(locationY, 0, YDimension - 1, "GetBlock: location is out of map bounds");
