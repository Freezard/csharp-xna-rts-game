...
namespace RTSgame.Utilities
{
    ...
    public class Array2D<T>
    {
        ...
        public delegate bool DelegateConditional(ref T data);
        ...
        /// <param name="upperLeft"></param>
        /// <param name="lowerRight"></param>
        public void ChangeValuesViaSpecifiedBoxWithConditional(DelegateChangeValue Del, DelegateConditional CondDel, Point upperLeft, Point lowerRight)
        {
            int xLeft = Calculations.Clamp(upperLeft.X, 0, XDimension - 1);
            int xRight = Calculations.Clamp(lowerRight.X, 0, XDimension - 1);
            int yUp = Calculations.Clamp(upperLeft.Y, 0, YDimension - 1);
            ...
            for (int x = upperLeft.X; x < lowerRight.X; x++)
            {
                for (int y = upperLeft.Y; y < lowerRight.Y; y++)
                {
                    if (CondDel.Invoke(ref Data[x, y]))
                        Del.Invoke(ref Data[x, y]);
                }
            }
        }
        ...
        /// <param name="upperLeft"></param>
        /// <param name="lowerRight"></param>
        public void ChangeValuesViaSpecifiedBox(DelegateChangeValueGetLocation Del, Point upperLeft, Point lowerRight)
        {
            int xLeft = Calculations.Clamp(upperLeft.X, 0, XDimension - 1);
            int xRight = Calculations.Clamp(lowerRight.X, 0, XDimension - 1);
            int yUp = Calculations.Clamp(upperLeft.Y, 0, YDimension - 1);
        ...
        /// <param name="upperLeft"></param>
        /// <param name="lowerRight"></param>
        public void ChangeValuesViaSpecifiedBoxWithConditional(DelegateChangeValueGetLocation Del, DelegateConditional CondDel, Point upperLeft, Point lowerRight)
        {
            int xLeft = Calculations.Clamp(upperLeft.X, 0, XDimension - 1);
            int xRight = Calculations.Clamp(lowerRight.X, 0, XDimension - 1);
            int yUp = Calculations.Clamp(upperLeft.Y, 0, YDimension - 1);
        ...
        /// <summary>
        /// Applies the function to all the values along the border
        /// of the entire datastructure. Border is one edge thinner on
        /// the south and east, which is nice to have for heightmaps where
        /// the height change bleeds over to the surrounding squares.
        /// </summary>
        /// <param name="Del">Method to be applied to all values</param>
        public void ChangeBorderValuesWithSlopeCompensation(DelegateChangeValue Del, int Thickness)
        {
            const int extraThinness = 1;

            int outerTop = 0;
            int outerBottom = YDimension - 1;
            int innerTop = Thickness;
            int innerBottom = outerBottom - Thickness + extraThinness;
            innerBottom = Math.Min(innerBottom, outerBottom);

            int outerLeft = 0;
            int outerRight = XDimension - 1;
            int innerLeft = Thickness;
            int innerRight = outerRight - Thickness + extraThinness;
            innerRight = Math.Min(innerRight, outerRight);
