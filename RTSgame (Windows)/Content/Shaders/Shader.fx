...
float4 TexturedPS(TexturedVSOut Input) : COLOR
{
	...
	float4 Ambient = float4(0.3, 0.3, 0.5, 0);

    float DirectionalIntensity = saturate(dot(LightDirection, Normal));

    float4 Diffuse = DirectionalIntensity * float4(1.5, 1.5, 1, 0);
	...
	float4 FinalColor = BaseColor * (Ambient + Diffuse) + specularIntensity * Specular;
...
//------- Technique: ShadowMap --------
 ...
 {
    return float4(Input.Depth - depthBias, 0, 0, 0);
 }
 ...
 float4 ShadowedPS(ShadowedVSOut Input) : COLOR
 {
	...
	float4 PointLightSum = float4(0, 0, 0, 0);
	...
	for (int i = 0; i < MaxLights; i++)
	{
		...
		PointLightSum += PointIntensity * lights[i].color;
	}

	//float3 HalfAngle = normalize(EyeVector + LightDirection);
	float3 HalfAngle = normalize(EyeVector + specularDirection);
	float Specular = pow(saturate(dot(Normal, HalfAngle)), shininess);
    ...
    if (Shadowdepth < Ourdepth)
    {
		...
		FinalColor = BaseColor * (Ambient + PointLightSum);
    }
	else
	{
		FinalColor = BaseColor * (Ambient + Diffuse + PointLightSum) + Specular * specularIntensity;
	}
	;
 ...
 technique Shadowed
 {
	pass Pass0
    {
        VertexShader = compile vs_3_0 ShadowedVS();
        PixelShader = compile ps_3_0 ShadowedPS();
    }
 }
 ...
 technique AnimatedShadowMap
 {
    pass Pass0
    {
        VertexShader = compile vs_3_0 AnimatedShadowMapVS();
        PixelShader = compile ps_3_0 ShadowMapPS();
    }
 }
 ...
 technique AnimatedShadowed
 {
	pass Pass0
    {
        VertexShader = compile vs_3_0 AnimatedShadowedVS();
        PixelShader = compile ps_3_0 ShadowedPS();
    }
 }
                                                  ...
                                                  float4x4 instanceTransform : BLENDWEIGHT)
{
 ...
 technique ShadowMapHardware
 {
    pass Pass0
    {
        VertexShader = compile vs_3_0 ShadowMapVSHardware1();
        PixelShader = compile ps_3_0 ShadowMapPS();
    }
 }
